...
HTML is not [Turing-complete](https://en.wikipedia.org/wiki/Turing_completeness), your browser interprets the HTML code via its own DOM API implementation under the hood. Regardless, imagine what a mess the web would be in if it never existed in the first place.

The declarative approach allows us to write a program without worrying about its **side effects**. Side effects such as rendering HTML elements, outputting to console, playing a sound, and etc. You know, [side effects](https://en.wikipedia.org/wiki/Side_effect_(computer_science)).
...
Actually, let's see its modern equivalent in [Haskell](https://www.haskell.org/). It has the same internal structure but with simpler syntax and a type annotation.
...
Not exactly. Explicit or not, it wouldn't have made any sense for the C compiler (nor for the JavaScript interpreter) if there were no statements to begin with. Simply because a C program is a sequence of commands separated by the **;** symbol &mdash; that's what a C program means. But for Haskell it makes perfect sense. What does a Haskell program _mean_ then?

# Denotational semantics

> "There are two ways of looking at a function: as an algorithm which will produce a value given an argument, or as a set of ordered argument-value pairs. The first view is "dynamic" or **operational**, in that it sees a function as a sequence of operations in time. The second view is "static" or **denotational**: the function is regarded as a fixed set of associations between arguments and the corresponding values." [Simon L. Peyton Jones](https://www.microsoft.com/en-us/research/wp-content/uploads/1987/01/slpj-book-1987-small.pdf)

Now take the bottom edge of the one seemingly unrelated triangle, the [_semiotic triangle_](https://en.wikipedia.org/wiki/Triangle_of_reference), and imagine that we have a magical box (like **⟦⟧**) which evaluates programmatic objects (like syntax) into mathematical objects (like values).

So you put any programmatic expression inside this box and on the other end of the equation you get something real, something that exists beyond thought, language and the known universe, like the integer 10 for instance.
...
It can be said that the denotation (or the meaning) of the `fib n` expression is the n-th Fibonacci number. The collection of such mathematical objects is called the **semantic domain**. Our semantic domain is in this case the Fibonacci sequence.

## Example: Arithmetic

>> Examples are adapted from [Eric Walkingshaw](https://web.engr.oregonstate.edu/~walkiner/)'s [CS581 lecture notes](https://web.engr.oregonstate.edu/~walkiner/teaching/cs581-fa20).

We first define a semantic **domain**: let's say, our domain is the set of all integers ℤ.
...
A language syntax doesn't necessarily have to be textual of course. You define an abstract syntax in Haskell using the `data` keyword, and a semantic domain, using `type`.
...
This is identical to the following TypeScript definition generated by [fp-ts-codegen](https://gcanti.github.io/fp-ts-codegen/).
...
In the following robot program, each `go` command instructs our robot to move n steps in the given direction. A semicolon is used for chaining a sequence of steps.
...
* A **Move** is expressed as a sequence of steps separated by semicolon.
...
Here is a function for summing up the total distance given a Step or a Move expression. The semantic domain is the set of all natural numbers.
...
In this case, we need a program to help find a target position given an initial (x,y) position to actually move a robot; meaning the semantic domain is the set of all _functions_ from one position to another: `ƒ : Pos → Pos`

We use **λ-calculus** (Lambda Calculus) for expressing function abstraction and application. It is explained in the next section, but for now you can think of those λ expressions on the right hand side as normal functions (e.g. `(x, y) => (x, y + k)`).
...
In fact, a lot of these **combinator** functions with one-letter names you've been seeing lately are coming from [combinatory logic](https://en.wikipedia.org/wiki/Combinatory_logic) (which is a variant of λ-calculus) and they are used to denote certain notions of computation such as recursions like the infamous [Y combinator](https://en.wikipedia.org/wiki/Fixed-point_combinator) does. [^y-combinator]

You'll hear that word a lot but it's important to understand in the beginning, the meaning of "combinator" is a more informal sense referring to the style of organizing libraries centered around the idea of combining things, and not strictly to combinatory logic.
